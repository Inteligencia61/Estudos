...
def date_to_sheets_serial(date_str: str) -> int:
    # Google Sheets usa base 1899-12-30 (mesma base do Excel)
...
def executar_analise_com_data(
    ...
    # ✅ coluna extra do seu sheet (casas): "Padrão"
    resultados["Padrão"] = inferir_padrao(df)
    return resultados[["NomeMedia", "Vaga", "Padrão", "Valor", "AmostraAnalisada", "Data"]]
def salvar_no_google_sheets(
    ...
    if "Data" in df.columns:
        df["Data"] = df["Data"].astype(str).str.lstrip("'").str.strip()
...
print(f"[INFO] Rodando LAGO SUL | Data={data_ref} | arquivo={input_file}")
