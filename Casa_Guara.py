...
# - FIX: filtro de "oferta" só é aplicado se o valor desejado existir na coluna
# =========================
import os
import pandas as pd
import numpy as np
...
def analisar_imovel_detalhado(
    ...
    for vaga_status, df_vaga in df.groupby(df["vagas"] > 0, observed=False):
        ...
        # =========================
        # FIX: oferta só filtra se existir
        # =========================
        if "oferta" in df_vaga.columns and oferta is not None:
            ...
            # Só aplica filtro se o valor desejado existir na coluna
            if wanted in set(col.dropna().unique()):
                filtro &= (col == wanted)
            # Caso contrário, ignora (ex.: coluna tem "PUBLICADO")
...
def executar_analise_com_data(
    ...
    resultados = analisar_imovel_detalhado(
        df=df,
        oferta="Venda",  # mantém; o FIX evita zerar se a coluna não tiver "VENDA"
...
RANGE_NAME = "Casa_Guara!A1"
TIPO_IMOVEL = "Casa"
BAIRRO = None
CIDADE = "GUARA"
QUADRA = None

print(f"[INFO] Rodando GUARÁ | Data={data_ref} | arquivo={input_file}")
